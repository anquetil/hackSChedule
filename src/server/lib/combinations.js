...
combinations.resolve = function(sections) {
	// union sections where blocks are equal
	var flipped = {};
	for (var id in sections) {
		var section = sections[id];
		var blocks = _.sortBy(section.blocks, ['day', 'start']);
		var key = blocks.map(function (block) {
			return block.day + block.start + block.end;
		}).join('/');
		if (!(key in flipped)) flipped[key] = [];
		flipped[key].push(id);
	}
	flipped = _.toArray(flipped);

	var resolvedData = {};
	for (var item of flipped) {
		var newKey = item.join('/');
		var newValue = sections[item[0]];
		resolvedData[newKey] = newValue;
	}
...
module.exports = function (coursedata) {
  return new Promise(function (resolve) {
		var sections = combinations.resolve(coursedata.sections);
